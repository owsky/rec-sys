...
def train_mf_als_mr(dataset: Dataset, seed: Optional[int] = None):
    ...
    # create the trainer
    als_trainer = AlsMrTrainer(model=mf, sparse_tr=dataset.sparse_tr, reg=reg)
    # train the model
    als_trainer.fit(val_loader=val_loader)
    # validate using test data loader
    rmse = mf.validate_prediction(te_loader)
    logger.info(f"MF ALS MR RMSE: {rmse}")
    ndcg = mf.validate_ranking(dataset.sparse_te.toarray())
    logger.info(f"MF ALS MR NDCG: {ndcg}")


def tune_mf_als_mr(dataset: Dataset, seed: Optional[int] = None):
    ...
    def tune_fn():
        with wandb.init():
            ...
            trainer = AlsMrTrainer(model=mf, sparse_tr=dataset.sparse_tr, reg=wandb.config.get("reg"))
